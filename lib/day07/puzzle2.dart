...
class Puzzle {
  ...
  int execute() {
    ...
    final totalSizeUsed =
        files.values.reduce((value, element) => value + element);
    final totalAvailableSpace = totalSize - totalSizeUsed;
    final requiredForUpdate = 30000000;
    final amountToFreeUp = requiredForUpdate - totalAvailableSpace;

    final potentialDirs = directorySizes.values
        .where((element) => element >= amountToFreeUp)
        .toList();
    potentialDirs.sort(min);
